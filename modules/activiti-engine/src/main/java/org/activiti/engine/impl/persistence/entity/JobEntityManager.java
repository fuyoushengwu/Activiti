...
public class JobEntityManager extends AbstractManager {
  ...
  public void send(MessageEntity message) {
    ...
    if (processEngineConfiguration.isAsyncExecutorEnabled()) {
      ...
      // before executing the job. This way, other async job executors can pick the job up after the
      // max lock time.
      Date dueDate =
          new Date(
      ...
      message.setLockExpirationTime(
          null); // was set before, but to be quickly picked up needs to be set to null
      ...
      // The job needs to be picked up as soon as possible. So the due date is now set to the
      // current time
  ...
  public void schedule(TimerEntity timer) {
    ...
    assert engineConfiguration != null;
    if (!engineConfiguration.isAsyncExecutorEnabled()
        && timer.getDuedate().getTime()
            <= (engineConfiguration.getClock().getCurrentTime().getTime())) {
  ...
  public void retryAsyncJob(JobEntity job) {
    ...
    try {
      ...
      // otherwise the job will be continuously be retried without delay (and thus seriously
      // stressing the database).
  ...
  @SuppressWarnings("unchecked")
  public List<Job> findJobsByTypeAndProcessDefinitionKeyNoTenantId(
  ...
  @SuppressWarnings("unchecked")
  public List<Job> findJobsByTypeAndProcessDefinitionKeyAndTenantId(
    ...
    return getDbSqlSession()
        .selectList("selectJobByTypeAndProcessDefinitionKeyAndTenantId", params);
  }

  @SuppressWarnings("unchecked")
  public List<Job> findJobsByTypeAndProcessDefinitionId(
  ...
  public void unacquireJob(String jobId) {
    ...
    params.put("id", jobId);
    params.put(
        "dueDate", new Date(getProcessEngineConfiguration().getClock().getCurrentTime().getTime()));
