...
public class SubProcessActivityBehavior extends AbstractBpmnActivityBehavior
  ...
  public void execute(ActivityExecution execution) throws Exception {
    ...
    ActivityImpl initialActivity =
        (ActivityImpl) activity.getProperty(BpmnParse.PROPERTYNAME_INITIAL);

    if (initialActivity == null) {
      throw new ActivitiException(
          "No initial activity found for subprocess " + execution.getActivity().getId());
    }
    ...
    if (initialActivity.getActivityBehavior() != null
        && initialActivity.getActivityBehavior()
            instanceof
            NoneStartEventActivityBehavior) { // embedded subprocess: only none start allowed
