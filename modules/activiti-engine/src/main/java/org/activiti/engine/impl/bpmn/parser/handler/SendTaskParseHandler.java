...
public class SendTaskParseHandler extends AbstractExternalInvocationBpmnParseHandler<SendTask> {
  ...
  protected void executeParse(BpmnParse bpmnParse, SendTask sendTask) {
    ...
    ActivityImpl activity =
        createActivityOnCurrentScope(bpmnParse, sendTask, BpmnXMLConstants.ELEMENT_TASK_SEND);
    ...
    if (StringUtils.isNotEmpty(sendTask.getType())) {
      if (sendTask.getType().equalsIgnoreCase("mail")) {
        ...
        activity.setActivityBehavior(
            bpmnParse
                ...
                .createMuleActivityBehavior(sendTask, bpmnParse.getBpmnModel()));
      } else if (sendTask.getType().equalsIgnoreCase("camel")) {
        activity.setActivityBehavior(
            bpmnParse
                .getActivityBehaviorFactory()
                .createCamelActivityBehavior(sendTask, bpmnParse.getBpmnModel()));
      }
      ...
      if (!bpmnParse.getOperations().containsKey(sendTask.getOperationRef())) {
        logger.warn(
            sendTask.getOperationRef() + " does not exist for sendTask " + sendTask.getId());
      } else {
        ...
        if (sendTask.getIoSpecification() != null) {
          IOSpecification ioSpecification =
              createIOSpecification(bpmnParse, sendTask.getIoSpecification());
