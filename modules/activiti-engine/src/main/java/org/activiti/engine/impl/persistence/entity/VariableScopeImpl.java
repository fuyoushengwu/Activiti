...
public abstract class VariableScopeImpl implements Serializable, VariableScope {
  ...
  protected Map<String, VariableInstanceEntity> variableInstances =
      null; // needs to be null, the logic depends on it for checking if vars were already fetched
  ...
  protected void updateVariableInstance(
    ...
    // Always check if the type should be altered. It's possible that the previous type is lower in
    // the type
    // checking chain (e.g. serializable) and will return true on isAbleToStore(), even though
    // another type
  ...
  protected VariableInstanceEntity createVariableInstance(
    ...
    VariableInstanceEntity variableInstance =
        VariableInstanceEntity.createAndInsert(variableName, type, value);
