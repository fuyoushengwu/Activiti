...
public abstract class MultiInstanceActivityBehavior extends FlowNodeActivityBehavior
  ...
  protected boolean isExtraScopeNeeded() {
    // special care is needed when the behavior is an embedded subprocess (not very clean, but it
    // works)
    return innerActivityBehavior
        instanceof org.activiti.engine.impl.bpmn.behavior.SubProcessActivityBehavior;
  }
  ...
  /**
   * Since the first loop of the multi instance is not executed as a regular activity, it is needed
   * to call the start listeners yourself.
   */
  protected void callCustomActivityStartListeners(ActivityExecution execution) {
    List<ExecutionListener> listeners =
        activity.getExecutionListeners(org.activiti.engine.impl.pvm.PvmEvent.EVENTNAME_START);

    List<ExecutionListener> filteredExecutionListeners =
        new ArrayList<ExecutionListener>(listeners.size());
      // Sad that we have to do this, but it's the only way I could find (which is also safe for
      ...
      CallActivityListenersOperation atomicOperation =
          new CallActivityListenersOperation(filteredExecutionListeners);
      Context.getCommandContext()
          .performOperation(atomicOperation, (InterpretableExecution) execution);
  }
  ...
  protected void callActivityEndListeners(ActivityExecution execution) {
    List<ExecutionListener> listeners =
        activity.getExecutionListeners(org.activiti.engine.impl.pvm.PvmEvent.EVENTNAME_END);
  ...
  /**
   * ACT-1339. Calling ActivityEndListeners within an {@link AtomicOperation} so that an
   * executionContext is present.
   *
   * @author Aris Tzoumas
   * @author Joram Barrez
   */
