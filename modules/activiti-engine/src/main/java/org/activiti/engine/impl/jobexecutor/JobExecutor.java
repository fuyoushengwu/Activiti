 ...
 *
 * <p>This component is responsible for performing all background work ({@link Job Jobs}) scheduled
 * by activiti.
 *
 * <p>You should generally only have one of these per Activiti instance (process engine) in a JVM.
 * In clustered situations, you can have multiple of these running against the same queue + pending
 * job list.
 *
 * @author Daniel Meyer
 * @author Joram Barrez
...
public abstract class JobExecutor {
    ...
    /**
     * To avoid deadlocks, the default for this is one. This way, in a clustered setup, multiple job
     * executors can acquire jobs without creating a deadlock due to fetching multiple jobs at once
     * and trying to lock them all at once.
     *
     * <p>In a non-clustered setup, this setting can be changed to any value > 0 without problems.
     *
     * <p>See https://activiti.atlassian.net/browse/ACT-1879 for more information.
     */
    ...
    /**
     * Called when a new job was added by the process engine to which this job executor belongs. This
     * is a hint, that for example the acquiring needs to start again when it would be sleeping.
     */
