...
public class HistoricTaskInstanceEntityManager extends AbstractManager {
  ...
  public long findHistoricTaskInstanceCountByQueryCriteria(
    ...
    if (getHistoryManager().isHistoryEnabled()) {
      return (Long)
          getDbSqlSession()
              .selectOne(
                  "selectHistoricTaskInstanceCountByQueryCriteria", historicTaskInstanceQuery);
    }
    return 0;
  }
  ...
  public List<HistoricTaskInstance> findHistoricTaskInstancesAndVariablesByQueryCriteria(
      HistoricTaskInstanceQueryImpl historicTaskInstanceQuery) {
    if (getHistoryManager().isHistoryEnabled()) {
      // paging doesn't work for combining task instances and variables due to an outer join, so
      // doing it in-memory
      ...
      if (historicTaskInstanceQuery.getTaskVariablesLimit() != null) {
        ...
        historicTaskInstanceQuery.setMaxResults(
            Context.getProcessEngineConfiguration().getHistoricTaskQueryLimit());
      }
  ...
  public HistoricTaskInstanceEntity findHistoricTaskInstanceById(String taskId) {
    ...
    if (getHistoryManager().isHistoryEnabled()) {
      return (HistoricTaskInstanceEntity)
          getDbSqlSession().selectOne("selectHistoricTaskInstance", taskId);
    }
    return null;
  }
          ...
          commandContext
              ...
              .dispatchEvent(
                  ActivitiEventBuilder.createEntityEvent(
                      ActivitiEventType.ENTITY_DELETED, historicTaskInstance));
        }
      }
    }
  }
