...
public class JPAEntityMappings {
  ...
  private final JPAEntityScanner entityScanner;

  public JPAEntityMappings() {
    classMetaDataMap = new HashMap<String, EntityMetaData>();
    entityScanner = new JPAEntityScanner();
  }
  ...
  private EntityMetaData scanClass(Class<?> clazz) {
    return entityScanner.scanClass(clazz);
  }
  ...
  public Object getIdValue(Object value, EntityMetaData metaData) {
    try {
      ...
      throw new ActivitiException(
          "Exception occured while getting value from id field/method on JPAEntity: "
  ...
  public Object createId(EntityMetaData metaData, String string) {
    ...
    // According to JPA-spec all primitive types (and wrappers) are supported, String, util.Date,
    // sql.Date,
