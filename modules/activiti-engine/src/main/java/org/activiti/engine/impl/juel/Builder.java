...
public class Builder implements TreeBuilder {
  ...
  public static enum Feature {
    /**
     * Method invocations as in <code>${foo.bar(1)}</code> as specified in JSR 245, maintenance
     * release 2. The method to be invoked is resolved at evaluation time by calling {@link
     * ELResolver#invoke(javax.el.ELContext, Object, Object, Class[], Object[])}.
     */
    METHOD_INVOCATIONS,
    /**
     * For some reason we don't understand, the specification does not allow to resolve <code>null
     * </code> property values. E.g. <code>${map[key]}</code> will always return <code>null</code>
     * if <code>key</code> evaluates to <code>null</code>. Enabling this feature will allow
     * <em>JUEL</em> to pass <code>null</code> to the property resolvers just like any other
     * property value.
     */
    NULL_PROPERTIES,
  ...
  public Tree build(String expression) throws TreeBuilderException {
    try {
      ...
      throw new TreeBuilderException(
          expression, e.position, e.encountered, e.expected, e.getMessage());
  ...
  public static void main(String[] args) {
    ...
    if (!tree.getFunctionNodes().iterator().hasNext()
