...
public class TimerStartEventJobHandler extends TimerEventHandler implements JobHandler {
  ...
  public void execute(
    ...
    if (TimerEventHandler.hasRealActivityId(configuration)) {
      startProcessInstanceWithInitialActivity(
          job, configuration, deploymentManager, commandContext);
    } else {
      startProcessDefinitionByKey(job, configuration, deploymentManager, commandContext);
    }
  }

  protected void startProcessInstanceWithInitialActivity(
      JobEntity job,
      String configuration,
      DeploymentManager deploymentManager,
      CommandContext commandContext) {
    ProcessDefinitionEntity processDefinition =
        deploymentManager.findDeployedProcessDefinitionById(job.getProcessDefinitionId());
    ...
    if (!deploymentManager.isProcessDefinitionSuspended(processDefinition.getId())) {
      ...
      ExecutionEntity processInstance =
          processDefinition.createProcessInstance(null, startActivity);
  ...
  protected void startProcessDefinitionByKey(
      JobEntity job,
      String configuration,
      DeploymentManager deploymentManager,
      CommandContext commandContext) {
    ...
    if (job.getTenantId() == null
        || ProcessEngineConfiguration.NO_TENANT_ID.equals(job.getTenantId())) {
      processDefinition =
          deploymentManager.findDeployedLatestProcessDefinitionByKey(processDefinitionKey);
    } else {
      processDefinition =
          deploymentManager.findDeployedLatestProcessDefinitionByKeyAndTenantId(
              processDefinitionKey, job.getTenantId());
    }
    ...
    try {
      if (!deploymentManager.isProcessDefinitionSuspended(processDefinition.getId())) {
        ...
        new StartProcessInstanceCmd<ProcessInstance>(
                processDefinitionKey, null, null, null, job.getTenantId())
