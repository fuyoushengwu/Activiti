...
public class TerminateEndEventActivityBehavior extends FlowNodeActivityBehavior {
  ...
  public void execute(ActivityExecution execution) throws Exception {
    ...
    if (terminateAll) {
      ...
      terminateProcessInstanceExecution(
          execution, terminateEndEventActivity, processInstanceExecution);
    } else {
  ...
  /**
   * Finds the parent execution that is a process instance. For a callactivity, this will be the
   * process instance representing the called process instance and NOT the root process instance!
   */
  ...
  protected ActivityExecution findRootProcessInstanceExecution(ExecutionEntity execution) {
    ...
    while (currentExecution.getParentId() != null
        || currentExecution.getSuperExecutionId() != null) {
  ...
  protected void terminateExecution(
    ...
    // Scope execution can already have been ended (for example when multiple seq flow arrive in the
    // same terminate end event)
  ...
  protected void terminateProcessInstanceExecution(
      ActivityExecution execution,
      ActivityImpl terminateEndEventActivity,
      ActivityExecution processInstanceExecution) {
    ...
    deleteProcessInstance(
        ...
        execution,
        "terminate end event (" + terminateEndEventActivity.getId() + ")");
  }

  protected void deleteProcessInstance(
      ExecutionEntity processInstanceExecution, ActivityExecution execution, String deleteReason) {
    ...
    Context.getCommandContext()
        ...
        .recordProcessInstanceEnd(
            processInstanceExecution.getId(), deleteReason, execution.getActivity().getId());
  }
  ...
  protected List<ExecutionEntity> orderExecutionsRootToLeaf(ExecutionEntity execution) {
    ...
    while (rootExecution.getParent() != null || rootExecution.getSuperExecution() != null) {
      rootExecution =
          rootExecution.getParent() != null
              ? rootExecution.getParent()
              : rootExecution.getSuperExecution();
    }

    return orderExecutionsRootToLeaf(rootExecution, new ArrayList<ExecutionEntity>());
  }

  protected List<ExecutionEntity> orderExecutionsRootToLeaf(
      ExecutionEntity rootExecution, List<ExecutionEntity> orderedExecutions) {
    orderedExecutions.add(rootExecution);

    // Children
    if (rootExecution.getExecutions() != null && !rootExecution.getExecutions().isEmpty()) {
  ...
  protected void endAllHistoricActivities(String processInstanceId) {
    ...
    Map<String, HistoricActivityInstanceEntity> historicActivityInstancMap =
        new HashMap<String, HistoricActivityInstanceEntity>();
    ...
    for (HistoricActivityInstance historicActivityInstance : historicActivityInstances) {
      historicActivityInstancMap.put(
    ...
    for (HistoricActivityInstanceEntity cachedHistoricActivityInstance :
      ...
      if (processInstanceId.equals(cachedHistoricActivityInstance.getProcessInstanceId())
        ...
        historicActivityInstancMap.put(
            cachedHistoricActivityInstance.getId(), cachedHistoricActivityInstance);
      }
    }

    for (HistoricActivityInstanceEntity historicActivityInstance :
        historicActivityInstancMap.values()) {
  ...
  protected void sendCancelledEvent(
      ActivityExecution execution,
      ActivityImpl terminateEndEventActivity,
      ActivityExecution scopeExecution) {
  ...
  protected void dispatchActivityCancelled(
      ActivityExecution execution, ActivityImpl activity, ActivityImpl causeActivity) {
    Context.getProcessEngineConfiguration()
        .getEventDispatcher()
        .dispatchEvent(
