...
public class ExecutionEntityManager extends AbstractManager {
  ...
  public void deleteProcessInstancesByProcessDefinition(
      String processDefinitionId, String deleteReason, boolean cascade, Boolean executeListeners) {
  ...
  public void deleteProcessInstance(
      String processInstanceId, String deleteReason, boolean cascade, boolean executeListeners) {
  ...
  private void deleteProcessInstanceCascade(
      ExecutionEntity execution,
      String deleteReason,
      boolean deleteHistory,
      boolean executeListeners) {
    ...
    List<ProcessInstance> subProcesses =
        processInstanceQuery.superProcessInstanceId(execution.getProcessInstanceId()).list();
    for (ProcessInstance subProcess : subProcesses) {
      deleteProcessInstanceCascade(
          (ExecutionEntity) subProcess, deleteReason, deleteHistory, executeListeners);
    }
  ...
  public ExecutionEntity findSubProcessInstanceBySuperExecutionId(String superExecutionId) {
    return (ExecutionEntity)
        getDbSqlSession().selectOne("selectSubProcessInstanceBySuperExecutionId", superExecutionId);
  }
  ...
  public List<ProcessInstance> findProcessInstanceAndVariablesByQueryCriteria(
    ...
    // doing it in-memory
    if (processInstanceQuery.getFirstResult() < 0 || processInstanceQuery.getMaxResults() <= 0) {
      return Collections.EMPTY_LIST;
    }

    int firstResult = processInstanceQuery.getFirstResult();
    int maxResults = processInstanceQuery.getMaxResults();

    // setting max results, limit to 20000 results for performance reasons
    if (processInstanceQuery.getProcessInstanceVariablesLimit() != null) {
      processInstanceQuery.setMaxResults(processInstanceQuery.getProcessInstanceVariablesLimit());
    } else {
      processInstanceQuery.setMaxResults(
          Context.getProcessEngineConfiguration().getExecutionQueryLimit());
    }
    processInstanceQuery.setFirstResult(0);
    ...
    List<ProcessInstance> instanceList =
        getDbSqlSession()
            .selectListWithRawParameterWithoutFilter(
                ...
                processInstanceQuery.getFirstResult(),
                processInstanceQuery.getMaxResults());
  ...
  @SuppressWarnings("unchecked")
  public List<ExecutionEntity> findEventScopeExecutionsByActivityId(
      String activityRef, String parentExecutionId) {
