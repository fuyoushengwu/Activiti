...
public class TaskEntityManager extends AbstractManager {
  ...
  public void deleteTasksByProcessInstanceId(
    ...
    List<TaskEntity> tasks =
        (List) getDbSqlSession().createTaskQuery().processInstanceId(processInstanceId).list();

    String reason =
        (deleteReason == null || deleteReason.isEmpty())
  ...
  public List<Task> findTasksAndVariablesByQueryCriteria(TaskQueryImpl taskQuery) {
    ...
    // paging doesn't work for combining task instances and variables due to an outer join, so doing
    // it in-memory
