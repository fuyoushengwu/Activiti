...
public class ScopeUtil {
  ...
  /**
   * Find the next scope execution in the parent execution hierarchy That method works different
   * than {@link
   * #findScopeExecutionForScope(org.activiti.engine.impl.persistence.entity.ExecutionEntity,
   * org.activiti.engine.impl.pvm.PvmScope)} which returns the most outer scope execution.
   *
  ...
  /**
   * returns the top-most execution sitting in an activity part of the scope defined by
   * 'scopeActivitiy'.
   */
  public static ExecutionEntity findScopeExecutionForScope(
      ExecutionEntity execution, PvmScope scopeActivity) {
   ...
   *
   * <p>create a new event scope execution under the parent of the given execution and move all
   * event subscriptions to that execution.
   *
   * <p>this allows us to "remember" the event subscriptions after finishing a scope
   */
  public static void createEventScopeExecution(ExecutionEntity execution) {

    ExecutionEntity eventScope =
        ScopeUtil.findScopeExecutionForScope(execution, execution.getActivity().getParent());

    List<CompensateEventSubscriptionEntity> eventSubscriptions =
        execution.getCompensateEventSubscriptions();
    ...
    if (!eventSubscriptions.isEmpty()) {
      ...
      CompensateEventSubscriptionEntity eventSubscription =
          CompensateEventSubscriptionEntity.createAndInsert(eventScope);
