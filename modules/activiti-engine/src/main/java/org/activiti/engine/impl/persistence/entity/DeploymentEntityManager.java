...
public class DeploymentEntityManager extends AbstractManager {
  ...
  public void deleteDeployment(String deploymentId, boolean cascade) {
    List<ProcessDefinition> processDefinitions =
        getDbSqlSession().createProcessDefinitionQuery().deploymentId(deploymentId).list();
      ...
      for (ProcessDefinition processDefinition : processDefinitions) {
        ...
        getProcessInstanceManager()
            .deleteProcessInstancesByProcessDefinition(
                processDefinitionId, "deleted deployment", cascade, false);
      }
    }
      ...
      List<EventSubscriptionEntity> eventSubscriptionEntities =
          getEventSubscriptionManager()
              .findEventSubscriptionsByTypeAndProcessDefinitionId(
                  null,
                  processDefinitionId,
                  processDefinition.getTenantId()); // null type ==> all types
    ...
    for (ProcessDefinition processDefinition : processDefinitions) {
      ...
      // If previous process definition version has a timer/message/signal start event, it must be
      // added
      ProcessDefinitionEntity latestProcessDefinition =
          findLatestProcessDefinition(processDefinition);

      // Only if the currently deleted process definition is the latest version, we fall back to the
      // previous start event type
        ...
        if (previousProcessDefinition != null) {
          ...
          ProcessDefinitionEntity resolvedProcessDefinition =
              Context.getProcessEngineConfiguration()
                  ...
                  .resolveProcessDefinition((ProcessDefinitionEntity) previousProcessDefinition);

          // Timer start
          List<TimerDeclarationImpl> timerDeclarations =
              (List<TimerDeclarationImpl>)
                  resolvedProcessDefinition.getProperty(BpmnParse.PROPERTYNAME_START_TIMER);
          ...
          if (signalEventDefinitions != null) {
            for (EventSubscriptionDeclaration eventDefinition : signalEventDefinitions) {
              ...
              } else if (eventDefinition.getEventType().equals("message")
                  && eventDefinition.isStartEvent()) {
                MessageEventSubscriptionEntity newSubscription =
                    new MessageEventSubscriptionEntity();
  ...
  protected ProcessDefinition findNewLatestProcessDefinitionAfterRemovalOf(
    ...
    List<ProcessDefinition> processDefinitions =
        getProcessDefinitionManager().findProcessDefinitionsByQueryCriteria(query, new Page(0, 1));
