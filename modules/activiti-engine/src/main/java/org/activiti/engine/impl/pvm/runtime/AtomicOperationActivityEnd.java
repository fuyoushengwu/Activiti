...
public class AtomicOperationActivityEnd extends AbstractEventAtomicOperation {
    ...
    if ((parentActivity != null) && (!parentActivity.isScope())) {
        ...
        Context.getProcessEngineConfiguration()
            ...
            .dispatchEvent(
                ActivitiEventBuilder.createEntityEvent(
                    ActivitiEventType.PROCESS_COMPLETED, execution));
      }
      ...
      if (concurrentRoot.getExecutions().size() == 1) {
        ...
        if (!lastConcurrent.isScope()) {
          ...
          // Make sure parent execution is re-activated when the last concurrent child execution is
          // active
  ...
  protected boolean isExecutionAloneInParent(InterpretableExecution execution) {
    ...
    for (InterpretableExecution other :
        (List<InterpretableExecution>) execution.getParent().getExecutions()) {
