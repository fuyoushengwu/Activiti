...
public class DefaultHistoryManager extends AbstractManager implements HistoryManager {
  ...
  public void recordProcessInstanceStart(ExecutionEntity processInstance) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      // Also record the start-event manually, as there is no "start" activity history listener for
      // this
  ...
  public void recordSubProcessInstanceStart(
    ...
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      HistoricProcessInstanceEntity historicProcessInstance =
          new HistoricProcessInstanceEntity(subProcessInstance);
      ...
      if (historicProcessInstance.getStartActivityId() == null) {
        historicProcessInstance.setStartActivityId(
            subProcessInstance.getProcessDefinition().getInitial().getId());
  ...
  public void recordActivityStart(ExecutionEntity executionEntity) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      if (executionEntity.getActivity() != null) {
        ...
        historicActivityInstance.setActivityName(
            (String) executionEntity.getActivity().getProperty("name"));
        historicActivityInstance.setActivityType(
            (String) executionEntity.getActivity().getProperty("type"));
        historicActivityInstance.setStartTime(
  ...
  public void recordActivityEnd(ExecutionEntity executionEntity) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      HistoricActivityInstanceEntity historicActivityInstance =
          findActivityInstance(executionEntity);
  ...
  protected void endHistoricActivityInstance(
      HistoricActivityInstanceEntity historicActivityInstance) {
    ...
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      HistoricActivityInstanceEntity historicActivityInstance =
          findActivityInstance(
              execution,
              activityId,
              false); // false -> no need to check the persistent store, as process just started
      if (historicActivityInstance != null) {
        endHistoricActivityInstance(historicActivityInstance);
      }
    }
   ...
   */
  protected HistoricActivityInstanceEntity findActivityInstance(
      ExecutionEntity execution, String activityId, boolean checkPersistentStore) {
    ...
    List<HistoricActivityInstanceEntity> cachedHistoricActivityInstances =
        getDbSqlSession().findInCache(HistoricActivityInstanceEntity.class);
    ...
    if (checkPersistentStore) {
      historicActivityInstances =
          new HistoricActivityInstanceQueryImpl(Context.getCommandContext())
  ...
  public void recordExecutionReplacedBy(
      ExecutionEntity execution, InterpretableExecution replacedBy) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      List<HistoricTaskInstanceEntity> cachedHistoricTaskInstances =
          getDbSqlSession().findInCache(HistoricTaskInstanceEntity.class);
      ...
      List<HistoricTaskInstanceEntity> historicTaskInstances =
          (List)
              new HistoricTaskInstanceQueryImpl(Context.getCommandContext())
                  .executionId(execution.getId())
                  .unfinished()
                  .list();
      ...
      List<HistoricVariableInstanceEntity> cachedHistoricVariableInstances =
          getDbSqlSession().findInCache(HistoricVariableInstanceEntity.class);
      for (HistoricVariableInstanceEntity cachedHistoricVariableInstance :
          cachedHistoricVariableInstances) {
      ...
      List<HistoricVariableInstanceEntity> historicVariableInstances =
          (List)
              new HistoricVariableInstanceQueryImpl(Context.getCommandContext())
                  .executionId(execution.getId())
                  .list();
  ...
  public void recordTaskCreated(TaskEntity task, ExecutionEntity execution) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      HistoricTaskInstanceEntity historicTaskInstance =
          new HistoricTaskInstanceEntity(task, execution);
  ...
  public void recordTaskClaim(String taskId) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      ...
      if (historicTaskInstance != null) {
        historicTaskInstance.setClaimTime(
            Context.getProcessEngineConfiguration().getClock().getCurrentTime());
      }
    }
  }
  ...
  public void recordHistoricDetailVariableCreate(
    ...
    if (isHistoryLevelAtLeast(HistoryLevel.FULL)) {
      ...
      if (useActivityId && sourceActivityExecution != null) {
        HistoricActivityInstanceEntity historicActivityInstance =
            findActivityInstance(sourceActivityExecution);
        if (historicActivityInstance != null) {
   ...
   */
  @Override
  public void reportFormPropertiesSubmitted(
      ExecutionEntity processInstance, Map<String, String> properties, String taskId) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      for (String propertyId : properties.keySet()) {
        String propertyValue = properties.get(propertyId);
        HistoricFormPropertyEntity historicFormProperty =
            new HistoricFormPropertyEntity(processInstance, propertyId, propertyValue, taskId);
  ...
  public void recordIdentityLinkCreated(IdentityLinkEntity identityLink) {
    ...
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)
      ...
      HistoricIdentityLinkEntity historicIdentityLinkEntity =
          new HistoricIdentityLinkEntity(identityLink);
