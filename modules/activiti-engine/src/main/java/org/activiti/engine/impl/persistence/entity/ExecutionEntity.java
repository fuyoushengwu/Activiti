...
public class ExecutionEntity extends VariableScopeImpl
  ...
  private static final Logger log = LoggerFactory.getLogger(ExecutionEntity.class);
   ...
   *
   * <ul>
   *   <li>an execution enters a nested scope
   ...
   * resolved to the replaced execution.
   *
  ...
  /**
   * next operation. process execution is in fact runtime interpretation of the process model. each
   * operation is a logical unit of interpretation of the process. so sequentially processing the
      ...
      Context.getProcessEngineConfiguration()
          ...
          .dispatchEvent(
              ActivitiEventBuilder.createEntityEvent(
                  ActivitiEventType.ENTITY_CREATED, createdExecution));
      Context.getProcessEngineConfiguration()
          .getEventDispatcher()
          .dispatchEvent(
              ActivitiEventBuilder.createEntityEvent(
                  ActivitiEventType.ENTITY_INITIALIZED, createdExecution));
    }
  ...
  public PvmProcessInstance createSubProcessInstance(PvmProcessDefinition processDefinition) {
    ...
    Map<String, Object> dataObjectVars =
        ((ProcessDefinitionEntity) processDefinition).getVariables();
  ...
  public void initialize() {
    ...
    List<TimerDeclarationImpl> timerDeclarations =
        (List<TimerDeclarationImpl>) scope.getProperty(BpmnParse.PROPERTYNAME_TIMER_DECLARATION);
    ...
    List<EventSubscriptionDeclaration> eventSubscriptionDeclarations =
    ...
    if (eventSubscriptionDeclarations != null) {
      for (EventSubscriptionDeclaration eventSubscriptionDeclaration :
          eventSubscriptionDeclarations) {
        if (!eventSubscriptionDeclaration.isStartEvent()) {
          EventSubscriptionEntity eventSubscriptionEntity =
              eventSubscriptionDeclaration.prepareEventSubscriptionEntity(this);
          if (getTenantId() != null) {
  ...
  /**
   * removes an execution. if there are nested executions, those will be ended recursively. if there
   * is a parent, this method removes the bidirectional relation between parent and this execution.
   */
  public void end() {
    isActive = false;
    isEnded = true;
  ...
  public void signal(String signalName, Object signalData) {
    ...
    SignallableActivityBehavior activityBehavior =
        (SignallableActivityBehavior) activity.getActivityBehavior();
    try {
      ...
      boolean isUserTask =
          (activityBehavior instanceof UserTaskActivityBehavior)
              || ((activityBehavior instanceof MultiInstanceActivityBehavior)
                  && ((MultiInstanceActivityBehavior) activityBehavior).getInnerActivityBehavior()
      ...
      if (!isUserTask
        ...
        Context.getProcessEngineConfiguration()
            .getEventDispatcher()
            .dispatchEvent(
                ActivitiEventBuilder.createSignalEvent(
  ...
  /**
   * @param fireActivityCompletionEvent This method can be called from other places (like {@link
   *     #takeAll(List, List)}), where the event is already fired. In that case, false is passed an
   *     no second event is fired.
   */
  ...
  public List<ActivityExecution> findInactiveConcurrentExecutions(PvmActivity activity) {
    List<ActivityExecution> inactiveConcurrentExecutionsInActivity =
    ...
    if (isConcurrent()) {
      ...
      for (ActivityExecution concurrentExecution : concurrentExecutions) {
        if (concurrentExecution.getActivity() != null
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "inactive concurrent executions in '{}': {}",
          activity,
  ...
  public void takeAll(
      List<PvmTransition> transitions, List<ActivityExecution> recyclableExecutions) {

    fireActivityCompletedEvent();

    transitions = new ArrayList<PvmTransition>(transitions);
    recyclableExecutions =
        (recyclableExecutions != null
            ? new ArrayList<ActivityExecution>(recyclableExecutions)
            : new ArrayList<ActivityExecution>());
    ...
    if ((transitions.size() == 1)
      ...
      log.debug(
          "activating the concurrent root {} as the single path of execution going forward",
          concurrentRoot);
      ...
      while (!transitions.isEmpty()) {
        ...
        if (recyclableExecutions.isEmpty()) {
          ...
          log.debug(
              "new {} with parent {} created to take transition {}",
              outgoingExecution,
              outgoingExecution.getParent(),
              outgoingTransition);
        } else {
      ...
      for (ExecutionEntity execution : executions) {
        ...
        if (!execution.isEnded) {
          if (activityId == null && otherActivityId != null
              ...
              || activityId != null && !otherActivityId.equals(activityId)) {
            return false;
          }
        }
      }
  ...
  public void destroyScope(String reason) {
    ...
    List<InterpretableExecution> executions =
        new ArrayList<InterpretableExecution>(getExecutions());
  ...
  private void removeEventScopes() {
    List<InterpretableExecution> childExecutions =
        new ArrayList<InterpretableExecution>(getExecutions());
    ...
    for (TaskEntity task : getTasks()) {
      if (replacedBy != null) {
        if (task.getExecution() == null || task.getExecution() != replacedBy) {
          // All tasks should have been moved when "replacedBy" has been set. Just in case tasks
          // where added,
  ...
  public void setReplacedBy(InterpretableExecution replacedBy) {
    ...
    List<TaskEntity> allTasks = new ArrayList<TaskEntity>(getTasks());
    ...
    List<VariableInstanceEntity> variables =
        commandContext.getVariableInstanceEntityManager().findVariableInstancesByExecutionId(id);
  ...
  public String toString() {
    if (isProcessInstanceType()) {
      ...
      return (isConcurrent ? "Concurrent" : "")
          + (isScope ? "Scope" : "")
          + "Execution["
          + getToStringIdentity()
          + "]";
  ...
  public List<CompensateEventSubscriptionEntity> getCompensateEventSubscriptions() {
    ...
    List<CompensateEventSubscriptionEntity> result =
        new ArrayList<CompensateEventSubscriptionEntity>(eventSubscriptions.size());
  ...
  /**
   * Adds an IdentityLink for this user with the specified type, but only if the user is not
   * associated with this instance yet.
   */
  ...
  public void setCachedEntityState(int cachedEntityState) {
    ...
    if (eventSubscriptions == null
        && !BitMaskUtil.isBitOn(cachedEntityState, EVENT_SUBSCRIPTIONS_STATE_BIT)) {
      eventSubscriptions = new ArrayList<EventSubscriptionEntity>();
    }
  }
  ...
  public int getCachedEntityState() {
    ...
    // Only mark a flag as false when the list is not-null and empty. If null, we can't be sure
    // there are no entries in it since
    // the list hasn't been initialized/queried yet.
    cachedEntityState =
        BitMaskUtil.setBit(cachedEntityState, TASKS_STATE_BIT, (tasks == null || !tasks.isEmpty()));
    cachedEntityState =
        BitMaskUtil.setBit(
            cachedEntityState,
    ...
    cachedEntityState =
        BitMaskUtil.setBit(cachedEntityState, JOBS_STATE_BIT, (jobs == null || !jobs.isEmpty()));
