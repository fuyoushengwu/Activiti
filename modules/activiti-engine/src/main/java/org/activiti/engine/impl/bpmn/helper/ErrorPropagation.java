...
public class ErrorPropagation {
          ...
          Context.getProcessEngineConfiguration()
              ...
              .dispatchEvent(
                  ActivitiEventBuilder.createEntityEvent(
                      ActivitiEventType.PROCESS_COMPLETED_WITH_ERROR_END_EVENT, execution));
        }
      }
  ...
  private static String findLocalErrorEventHandler(ActivityExecution execution, String errorCode) {
    ...
    while (scope != null) {
      ...
      List<ErrorEventDefinition> definitions =
          (List<ErrorEventDefinition>)
              scope.getProperty(BpmnParse.PROPERTYNAME_ERROR_EVENT_DEFINITIONS);
      if (definitions != null) {
  ...
  private static void executeCatch(
      String errorHandlerId, ActivityExecution execution, String errorCode) {
    ...
    if (catchingScope instanceof PvmProcessDefinition) {
      executeEventHandler(
          errorHandler, ((ExecutionEntity) execution).getProcessInstance(), errorCode);
      ...
      if (currentActivity.getId().equals(catchingScope.getId())) {
        ...
        // Follow parents up until matching scope can't be found anymore (needed to support for
        // multi-instance)
      ...
      if (matchingParentFound && leavingExecution != null) {
        ...
        throw new ActivitiException(
            "No matching parent execution for activity " + errorHandlerId + " found");
      }
    }
  }

  private static void executeEventHandler(
      ActivityImpl borderEventActivity, ActivityExecution leavingExecution, String errorCode) {
    if (Context.getProcessEngineConfiguration() != null
        && Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
      Context.getProcessEngineConfiguration()
          .getEventDispatcher()
          .dispatchEvent(
              ActivitiEventBuilder.createErrorEvent(
                  ...
                  borderEventActivity.getId(),
                  errorCode,
                  leavingExecution.getId(),
                  leavingExecution.getProcessInstanceId(),
    ...
    Context.getCommandContext()
        .getHistoryManager()
        .recordActivityEnd((ExecutionEntity) leavingExecution);

    if (borderEventActivity.getActivityBehavior()
        instanceof EventSubProcessStartEventActivityBehavior) {
      ...
      execution.performOperation(
          AtomicOperation.ACTIVITY_START); // make sure the listeners are invoked!
    } else {
  ...
  public static boolean mapException(
      Exception e, ActivityExecution execution, List<MapExceptionEntry> exceptionMap)
      throws Exception {
