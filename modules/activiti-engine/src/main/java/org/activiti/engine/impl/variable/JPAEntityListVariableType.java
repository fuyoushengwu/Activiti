...
public class JPAEntityListVariableType implements VariableType, CacheableVariable {
  ...
  public boolean isAbleToStore(Object value) {
    ...
    if (value instanceof List<?>) {
      ...
      if (!list.isEmpty()) {
        // We can only store the list if we are sure it's actually a list of JPA entities. In case
        // the
          ...
          canStore =
              entity != null
                  ...
                  && mappings
                      ...
                      .equals(entityClass);
          if (!canStore) {
            // In case the object is not a JPA entity or the class doesn't match, we can't store the
            // list
            break;
          }
        }
