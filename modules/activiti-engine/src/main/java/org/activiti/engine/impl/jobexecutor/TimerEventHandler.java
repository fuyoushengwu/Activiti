...
public class TimerEventHandler {
  ...
  /**
   * Before Activiti 5.21, the jobHandlerConfiguration would have as activityId the process
   * definition key (as only one timer start event was supported). In >= 5.21, this changed and in
   * >= 5.21 the activityId is the REAL activity id. It can be recognized by having the
   * 'processDefinitionKey' in the configuration. A < 5.21 job would not have that.
   */
  public static boolean hasRealActivityId(String jobHandlerConfiguration) {
    try {
      ...
      if (processDefinitionKey != null) {
        return !processDefinitionKey.toString().isEmpty();
      }
    } catch (JSONException ex) {
      return false;
    }
